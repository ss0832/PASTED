...
def parse_element_spec(spec: str) -> list[str]:
    ...
    for z in sorted(z_set):
        if z not in _Z_TO_SYM:
            raise ValueError(
                f"Z={z} not supported (supported range: {_ALL_Z[0]}–{_ALL_Z[-1]})"
            )
...
def parse_filter(f: str) -> tuple[str, float, float]:
    ...
    if metric not in ALL_METRICS:
        raise ValueError(
            f"Unknown metric {metric!r}. Valid metrics: {sorted(ALL_METRICS)}"
        )
...
def validate_charge_mult(
    ...
    if n_e <= 0:
        return False, (
            f"n_electrons={n_e} (total_Z={total_z}, charge={charge:+d})."
        )
    n_up = mult - 1
    if (n_e % 2) != (n_up % 2):
        return False, (
    ...
    comp = " ".join(
        f"{s}:{c}" for s, c in sorted(Counter(atoms_list).items())
    )
    return True, (
