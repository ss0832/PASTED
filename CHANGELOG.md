...
- **`pasted._ext._graph_core`** — new C++17 extension replacing the three
  dominant O(N²) metrics bottlenecks with O(N·k) FlatCellList-based
  implementations (k = mean bonded-pair count per atom, ~constant):

  | Metric | Python (N=1000) | C++ (N=1000) | Speedup |
  |---|---:|---:|---:|
  | `ring_fraction` | ~90 ms | ~3 ms | **~30×** |
  | `charge_frustration` | ~88 ms | ~3 ms | **~30×** |
  | `graph_lcc` / `graph_cc` | ~35 ms | ~4 ms | **~9×** |
  | metrics TOTAL (N=1000) | ~382 ms | ~20 ms | **~19×** |

  All four metrics (`graph_lcc`, `graph_cc`, `ring_fraction`,
  `charge_frustration`) are computed in a single C++ call that builds the
  `FlatCellList` and bonded-pair adjacency list only once.  A `HAS_GRAPH`
...
- **`bond_strain_rms`** removed from `ALL_METRICS`, `compute_all_metrics`,
  and the public API (`pasted.__init__`).  The function definition in
  `_metrics.py` is also deleted.
  ...
  under normal usage (`cov_scale = 1.0`).  The metric carries no information
  about the generated structures.

  Migration: remove `"bond_strain_rms"` from any `--filter` or `objective`
  dict.  `ALL_METRICS` now has 12 keys (down from 13).

### Added (continued)

- **`moran_I_chi`** — Moran's I spatial autocorrelation for Pauling
  electronegativity, implemented in C++ (`_graph_core`) via FlatCellList:

  .. math::

      I = (N/W) \cdot \sum_{i\neq j} w_{ij}(\chi_i-\bar\chi)(\chi_j-\bar\chi)
          / \sum_i (\chi_i-\bar\chi)^2

  where :math:`w_{ij}=1` when :math:`d_{ij}\leq` *cutoff* (existing cutoff
  parameter, no new API parameter needed).

  Physical interpretation:

  * I ≈ 0: random spatial arrangement of electronegativity (target for
    disordered structures generated by PASTED)
  * I > 0: atoms of similar electronegativity cluster spatially
...
- **Noble gas Pauling electronegativity values updated** in `_PAULING_EN`:
  ...
  |---|---|---|---|
  | He, Ne, Ar, Rn | 4.0 | 4.0 | unchanged — no compounds known |
  | **Kr** | 4.0 | **3.0** | KrF₂ exists; Allen/Allred-Rochow estimate |
  | **Xe** | 4.0 | **2.6** | XeF₂/XeO₃ well characterised; literature estimate |

- `compute_all_metrics` docstring now derives the metric count from
  ``len(ALL_METRICS)`` rather than a hardcoded number, eliminating
  future update churn.
- `ALL_METRICS` increased from 12 to 13 keys (added `moran_I_chi`).
- `graph_metrics_cpp` now computes all five graph/ring/charge/Moran metrics
  in a single FlatCellList pass; the separate `moran_I_chi_cpp` call has
  been inlined, halving the number of spatial-index builds per
  `compute_all_metrics` invocation.
- CLI `--filter` help text and `docs/cli.md` updated with `moran_I_chi`
  example: ``--filter "moran_I_chi:-0.1:0.1"`` selects spatially random
  electronegativity arrangements.
- `ALL_METRICS` reduced from 13 to 12 keys.
