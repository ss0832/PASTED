...
| `--n-threads N` | int | None | Number of OpenMP threads for C++ extensions (Linux only). Default: all available cores (`OMP_NUM_THREADS`). No effect when `HAS_OPENMP` is `False` |
